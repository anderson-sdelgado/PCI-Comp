...
import br.com.usinasantafe.pci.infra.models.retrofit.variable.headerRoomModelToHeaderRetrofitModel
import br.com.usinasantafe.pci.infra.models.retrofit.variable.respRoomModelToRespRetrofitModel
...
class ICheckListRepository @Inject constructor(
    ...
    override suspend fun getIdOSHeaderOpen(): Result<Int> {
        val result = headerRoomDatasource.getIdOSByStatusOpen()
        if (result.isFailure)
            return resultFailure(
                context = getClassAndMethod(),
                cause = result.exceptionOrNull()!!
    ...
    override suspend fun allHeader(): Result<List<Header>> {
        try {
            val result = headerRoomDatasource.all()
            if (result.isFailure)
                return resultFailure(
                    context = getClassAndMethod(),
                    cause = result.exceptionOrNull()!!
    ...
    override suspend fun deleteHeader(id: Int): Result<Boolean> {
        val result = headerRoomDatasource.delete(id)
        if (result.isFailure)
            return resultFailure(
                context = getClassAndMethod(),
                cause = result.exceptionOrNull()!!
    ...
    override suspend fun saveResp(resp: Resp): Result<Boolean> {
        try {
            val resultGetOpen = headerRoomDatasource.getIdByStatusOpen()
            ...
            val idHeader = resultGetOpen.getOrNull()!!
    ...
    override suspend fun listRespByHeaderOpen(): Result<List<Resp>> {
        try {
            ...
            val idHeader = resultGetId.getOrNull()!!
            val resultList = respRoomDatasource.listByIdHeader(idHeader)
    ...
    override suspend fun sendNote(
        ...
        try {
            ...
            val respRoomModelList = resultListResp.getOrNull()!!
            val idHeaderList = respRoomModelList.map { it.idHeader }.distinct()
            ...
            val headerRoomModelList = resultListHeader.getOrNull()!!
            val modelRetrofitOutputList = headerRoomModelList.map {
                it.headerRoomModelToHeaderRetrofitModel(
                    number = number,
                    respList = respRoomModelList.filter { resp ->
                        resp.idHeader == it.id
                    }.map { resp -> resp.respRoomModelToRespRetrofitModel() }
                )
            }
            val resultSend = checkListRetrofitDatasource.send(
                token = token,
                retrofitModelOutputList = modelRetrofitOutputList
            )
            ...
            val headerModelRetrofitInputList = resultSend.getOrNull()!!
            for (headerModelRetrofitInput in headerModelRetrofitInputList) {
                for (respRetrofitModel in headerModelRetrofitInput.respList) {
                    val resultSetIdServAndSentById = respRoomDatasource.setIdServAndSentById(
                        id = respRetrofitModel.id,
                        idServ = respRetrofitModel.idServ
                    )
                    if (resultSetIdServAndSentById.isFailure) {
                        return resultFailure(
                            context = getClassAndMethod(),
                            cause = resultSetIdServAndSentById.exceptionOrNull()!!
                        )
                    }
                }
                val resultSetIdServById = headerRoomDatasource.setIdServById(
                    id = headerModelRetrofitInput.id,
                    idServ = headerModelRetrofitInput.idServ
                )
                if (resultSetIdServById.isFailure) {
                    return resultFailure(
                        context = getClassAndMethod(),
                        cause = resultSetIdServById.exceptionOrNull()!!
                    )
                }
            }
